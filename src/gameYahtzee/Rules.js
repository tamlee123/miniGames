class Rule {
  constructor(params) {
    //to put all properties in params
  ...
  //times value appears in dice
  count(dice, val) {
    return dice.filter((d) => d === val).length;
  }
}

//return sum of dice of that val-used for rule like 'sum of all ones'
...
// use for "3-of-kind and 2-of-kind"
...
// use for SmallStraights
class SmallStraight extends Rule {
  evalRoll = (dice) => {
    const d = new Set(dice);
    //straigh can be 234 + either 1 or 5
...
//score number for every that number in the row
